...
function Birds() {
    ...
    useEffect(() => {
        if (productBirds.length > 0) {
            const prices = productBirds.map((product) => parseFloat(product.price)).filter((price) => !isNaN(price));
            if (prices.length > 0) {
            ...
            setFilteredProducts(productBirds);
        }
    }, [productBirds]);

    // Apply filters
    useEffect(() => {
        const filtered = productBirds.filter((product) => {
    ...
    }, [search, selectedMaterials, selectedColors, selectedCategories, priceRange, productBirds]);
    ...
    return (
        <>
            <div>
                <h1 className="text-4xl text-center mb-14 mt-8 font-bold">Products for birds</h1>
            </div>
                    ...
                    <div className="filter_container">
                        ...
                        <div className="mt-4">
                            ...
                            <div className="flex flex-col mt-2">
                                {[...new Set(productBirds.map((product) => product.material).filter(Boolean))].map(
                                    (material, index) => (
                        ...
                        <div className="mt-4">
                            <span>Colors</span>
                            <div className="flex flex-col mt-2">
                                {[...new Set(productBirds.map((product) => product.color).filter(Boolean))].map(
                                    (color, index) => (
                                        <label key={index} className="flex items-center mb-2">
                                            <input
                                                type="checkbox"
                        ...
                        <div className="mt-4">
                            <span>Category</span>
                            <div className="flex flex-col mt-2">
                                {[...new Set(productBirds.map((product) => product.category).filter(Boolean))].map(
                                    (category, index) => (
                                        <label key={index} className="flex items-center mb-2">
                                            <input
                                                type="checkbox"
