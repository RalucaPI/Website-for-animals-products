...
function Dogs() {
    ...
    useEffect(() => {
        if (productDogs.length > 0) {
            const prices = productDogs.map((product) => parseFloat(product.price)).filter((price) => !isNaN(price));
            if (prices.length > 0) {
            ...
            setFilteredProducts(productDogs);
        }
    }, [productDogs]);

    // Apply filters
    useEffect(() => {
        const filtered = productDogs.filter((product) => {
    ...
    }, [search, selectedMaterials, selectedColors, selectedCategories, priceRange, productDogs]);
    ...
    return (
        <>
            <div>
                <h1 className="text-4xl text-center mb-14 mt-8 font-bold">Products for dogs</h1>
            </div>
                    ...
                    <div className="filter_container">
                        ...
                        <div className="mt-4">
                            ...
                            <div className="flex flex-col mt-2">
                                {[...new Set(productDogs.map((product) => product.material).filter(Boolean))].map(
                                    (material, index) => (
                        ...
                        <div className="mt-4">
                            <span>Colors</span>
                            <div className="flex flex-col mt-2">
                                {[...new Set(productDogs.map((product) => product.color).filter(Boolean))].map(
                                    (color, index) => (
                                        <label key={index} className="flex items-center mb-2">
                                            <input
                                                type="checkbox"
                        ...
                        <div className="mt-4">
                            <span>Category</span>
                            <div className="flex flex-col mt-2">
                                {[...new Set(productDogs.map((product) => product.category).filter(Boolean))].map(
                                    (category, index) => (
                                        <label key={index} className="flex items-center mb-2">
                                            <input
                                                type="checkbox"
